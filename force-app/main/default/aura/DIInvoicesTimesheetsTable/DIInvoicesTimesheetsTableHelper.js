({
    ...
    proccessData: function(component, dataList) {
        dataList = dataList.map((rowData)  => {
            ...
            rowData.timePeriod = (startDate.getDate() + ' ' + distdlib.date.months[startDate.getMonth()].label) + ' ' + startDate.getFullYear() + ' - ' + 
                                (endDate.getDate() + ' ' + distdlib.date.months[endDate.getMonth()].label) + ' ' + startDate.getFullYear();

            rowData.totalHoursLabel = distdlib.time.convert(rowData.totalMinutes, 'mm', '*:*');

            rowData.totalAmountLabel = (distdlib.isNumber(rowData.totalAmount)) ? rowData.totalAmount.toFixed(2) : (rowData.totalAmount + '');
            
            rowData.rateLabel = rowData.rate + '';
    ...
    getTotal: function(component) {
        ...
        this.sendRequest(component, 'c.getTotalItemsCount', params)   
        .then(data => {
